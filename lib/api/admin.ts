...
export const mediaApi = {
  getAll: (params?: { page?: number; limit?: number; search?: string }) =>
    api.get<PaginatedResponse<Media>>('/v1/media', { params }),
  
  getById: (id: string) => api.get<{ data: Media }>(`/v1/media/${id}`),
  
  upload: (file: File) => {
    const formData = new FormData()
    formData.append('file', file)
    return api.post<{ data: Media }>('/v1/media/upload', formData, {
  ...
  uploadMultiple: (files: File[]) => {
    ...
    return api.post<{ data: Media[] }>('/v1/media/upload-multiple', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })
  },
